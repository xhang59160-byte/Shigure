...
- **动态单位**（可作目标，也可在条件引用）：解析为一个队伍槽位（`1`~`30`）。选择器类型包括——`生命值最低`、`带任一光环且血最低`、`不带某光环且血最低`、`带某光环且血最低`、`光环层数等于某值且血最低`、`按职责取首/末`、`按职责且不带某光环`、`带某光环(持续最久)`、`带某驱散类型`。参数（血量阈值、职责、光环、光环层数、驱散类型等）按所选类型自适应显示，光环候选来自当前职业/专精的 `group` 字段。编辑弹窗可选填“名称2 (生命值)”：填了之后，该单位解析槽位的“生命值”会暴露成一个同名数值条件字段——如取名 `最低血量` 后条件可直接写 `最低血量 < 50`（等价于 `单位名.生命值 < 50`）；单位未解析时该字段视为缺失（关系比较不命中）。
...
- 在“条件”里可引用：`单位名.字段`（如 `最低血量单位.生命值 < 50`，读该槽位的队伍字段）、裸 `单位名`（解析到槽位即为真，可用于存在性判断）、单位的“名称2”（数值，等于该单位的生命值）、以及数量名（如 `低血量人数 >= 3`）。这些都会出现在可视化条件编辑器的字段下拉中。

模块 JSON 中以 `Units`、`Counts` 数组保存（单位的名称2 存为 `HealthName`，未填则省略），规则用 `UnitName` 字段引用动态单位（与原有数字 `Unit` 互斥）。旧模块没有这些键也能正常加载。名称与名称2 不能为纯数字、不能含 `.` 或 `$`，且不能与现有状态/队伍字段或其它单位/数量名重复。
